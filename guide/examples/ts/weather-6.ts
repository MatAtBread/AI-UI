...
const Location = input.extended({
  declare: {
    get geo() {
      // We have to cast from `this` as getters don't respect ThisType as of TS 5.3
      const i = (this as unknown as ReturnType<typeof input>);
      return i.when('change').map(_ => getGeoInfo(i.value).then(g => { 
