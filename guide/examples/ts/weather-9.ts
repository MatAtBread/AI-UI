...
const WeatherForecast = Chart.extended({
  ...
  constructed() {
    this.geo.consume!(g => {
      this.style.opacity = '0.2';
      if (g) {
        /* Note: we could use `await` here but choose not to in order to complete
        construction of the chart even with no data for display purposes */
...
const Location = input.extended({
  ...
  constructed() {
    this.when("change").consume(async ()=>{
      try {
        this.disabled = true;
