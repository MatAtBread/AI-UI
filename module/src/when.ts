...
// Note: this slows down typescript. Simply defining `CSSIdentifier = string` is quicker, but prevents when validating sub-IDSs or selector formatting
...
type IsValidWhenSelector<S> = S extends ValidWhenSelector ? S : never
...
function isValidWhenSelector(what: WhenParameter): what is ValidWhenSelector {
  if (!what)
...
function containerAndSelectorsMounted(container: Element, selectors?: string[]) {
  ...
  function allSelectorsPresent(missing: string[]): Promise<void> {
