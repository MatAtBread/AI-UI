...
${_}`)),T.next().then(A).catch(M)}},M=k=>{E.warn("Dynamic attribute error",k,p,r,_,z(a)),T.return?.(k),a.appendChild(I({error:k}))},H=y.valueOf();H!==void 0&&H!==y&&!K(H)?A({done:!1,value:H}):T.next().then(A).catch(M)}function te(y,p){if(y instanceof Node)E.info("Having DOM Nodes as properties of other DOM Nodes is a bad idea as it makes the DOM tree into a cyclic graph. You should reference nodes by ID or via a collection such as .childNodes",p,z(y)),r[p]=y;else if(!(p in r)||r[p]===y||Array.isArray(r[p])&&r[p].length!==y.length)if(y.constructor===Object||y.constructor===Array){let T=new y.constructor;m(T,y),r[p]=T}else r[p]=y;else Object.getOwnPropertyDescriptor(r,p)?.set?r[p]=y:m(r[p],y)}}(a,f)}function w(a){for(let f=a.constructor;f;f=f.super)if(f===this)return!0;return!1}function v(a){let f=typeof a!="function"?p=>Object.assign({},a,p):a,m=Date.now().toString(36)+(lt++).toString(36)+Math.random().toString(36).slice(2),r=f({[fe]:m});r.styles&&(j.appendChild(s.createTextNode(r.styles+`
`)),s.head.contains(j)||s.head.appendChild(j));let d=Object.assign((p,...T)=>{let P=he(p),V=[],_={[de]:(P?V:p[de])??V},A=P?this(_,p,...T):this(_,...T);A.constructor=d;let M=f({[fe]:m});if(_[de].push(M),S){let C=function(Q,L){for(let U=Q;U;U=U.super)if(U.definition?.declare&&L in U.definition.declare)return!0;return!1};var k=C;if(M.declare){let Q=Object.keys(M.declare).filter(L=>L in A||C(this,L));Q.length&&E.log(`Declared keys '${Q}' in ${d.name} already exist in base '${this.valueOf()}'`)}if(M.override){let Q=Object.keys(M.override).filter(L=>!(L in A)&&!(l&&L in l)&&!C(this,L));Q.length&&E.log(`Overridden keys '${Q}' in ${d.name} do not exist in base '${this.valueOf()}'`)}}O(A,M.declare,!0),O(A,M.override);let H=new Set;if(M.iterable&&Object.keys(M.iterable).forEach(C=>{C in A?(E.log(`Ignoring attempt to re-define iterable property "${C}" as it could already have consumers`),H.add(C)):xe(A,C,M.iterable[C])}),_[de]===V){P||F(A,p);for(let L of V){let U=L?.constructed?.call(A);he(U)&&A.append(...x(U))}let C={},Q=!1;for(let L of V)if(L.iterable)for(let U of Object.keys(L.iterable)){let Me=!P&&U in p;if(H.has(U)&&Me||!(Me&&(!Y(p[U])||!K(p[U])))){let Ce=A[U]?.valueOf();Ce!==void 0&&(C[U]=Ce,Q=!0)}}Q&&Object.assign(A,C)}return A},{super:this,definition:Object.assign(r,{[fe]:m}),extended:v,valueOf:()=>{let p=[...Object.keys(r.declare||{}),...Object.keys(r.iterable||{})];return`${d.name}: {${p.join(", ")}}
 \u21AA ${this.valueOf()}`}});Object.defineProperty(d,Symbol.hasInstance,{value:w,writable:!0,configurable:!0});let R={};(function p(T){T?.super&&p(T.super);let P=T.definition;P&&(O(R,P?.override),O(R,P?.declare))})(this),O(R,r.override),O(R,r.declare),Object.defineProperties(d,Object.getOwnPropertyDescriptors(R));let te=R&&"className"in R&&typeof R.className=="string"?R.className:m,y=S?new Error().stack?.split(`
`)[2]??"":"";if(Object.defineProperty(d,"name",{value:"<ai-"+te.replace(/\s+/g,"-")+y+">"}),S){let p=Object.keys(r).filter(T=>!["styles","ids","constructed","declare","override","iterable"].includes(T));p.length&&E.log(`${d.name} defines extraneous keys '${p}', which are unknown`)}return d}let G={createElement(a,f,...m){return a===G.createElement?x(...m):typeof a=="function"?a(f,m):typeof a=="string"&&a in G?G[a](f,m):a instanceof Node?a:I({error:new Error("Illegal type in createElement:"+a)})}};function N(a){if(G[a])return G[a];let f=(r,...g)=>{if(he(r)&&(g.unshift(r),r={}),!he(r)){if(r.debugger){debugger;delete r.debugger}let d=o?s.createElementNS(o,a.toLowerCase()):s.createElement(a);return d.constructor=f,O(d,b),F(d,r),d.append(...x(...g)),d}},m=Object.assign(f,{super:()=>{throw new Error("Can't invoke native elemenet constructors directly. Use document.createElement().")},extended:v,valueOf(){return`TagCreator: <${o||""}${o?"::":""}${a}>`}});return Object.defineProperty(f,Symbol.hasInstance,{value:w,writable:!0,configurable:!0}),Object.defineProperty(f,"name",{value:"<"+a+">"}),G[a]=m}return i.forEach(N),G};function ut(e,t,n){let o=new WeakSet;function i(u){for(let l of u)t==="addedNodes"===l.isConnected&&(i(l.childNodes),o.add(l),n&&"onRemovedFromDOM"in l&&typeof l.onRemovedFromDOM=="function"&&l.onRemovedFromDOM())}return new MutationObserver(u=>{u.forEach(function(l){l.type==="childList"&&l[t].length&&i(l[t])})}).observe(e,{subtree:!0,childList:!0}),function(u){return o.has(u)}}return qe(dt);})();
