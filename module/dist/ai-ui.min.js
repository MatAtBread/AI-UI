...
${V}`)),T.next().then(w).catch(O)}},O=E=>{v.warn("Dynamic attribute error",E,g,r,V,q(a)),T.return?.(E),a.appendChild(de({error:E}))},R=y.valueOf();R!==void 0&&R!==y&&!W(R)?w({done:!1,value:R}):T.next().then(w).catch(O)}function Q(y,g){if(y instanceof Node)v.info("Having DOM Nodes as properties of other DOM Nodes is a bad idea as it makes the DOM tree into a cyclic graph. You should reference nodes by ID or via a collection such as .childNodes",g,q(y)),r[g]=y;else if(!(g in r)||r[g]===y||Array.isArray(r[g])&&r[g].length!==y.length)if(y.constructor===Object||y.constructor===Array){let T=new y.constructor;h(T,y),r[g]=T}else r[g]=y;else Object.getOwnPropertyDescriptor(r,g)?.set?r[g]=y:h(r[g],y)}}(a,c)}function C(a){for(let c=a.constructor;c;c=c.super)if(c===this)return!0;return!1}function j(a){let c=typeof a!="function"?g=>Object.assign({},a,g):a,h=Date.now().toString(36)+(ot++).toString(36)+Math.random().toString(36).slice(2),r=c({[oe]:h});r.styles&&(fe.appendChild(document.createTextNode(r.styles+`
